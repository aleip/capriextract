...
mapping <- function(scope = "capdiscapreg", 
  ...
  for(page in 1:npages){
    ...
    if (is.null(cuts_hard)){
      if (length(crps_over_sd) != 0){
        ...
        if(cuts_const %in% c("Y", "Yes") & page == 1){
          ...
          capridat_dcst <- dcast(capridat, RALL + ROWS + Y ~ COLS, drop = TRUE, value.var = "VALUE", sum, na.rm=TRUE)
          head(capridat_dcst)
          
          cuts_1 <- stats::quantile(capridat_dcst[crps_over_sd][capridat_dcst[crps_over_sd] > 0], probs = seq(0, 1, 1/n_cuts), na.rm = T)
          lev_1 <- levels(cut(as.numeric(unlist(capridat_dcst[crps_over_sd])), cuts_1))
          
          crps_0 <- crps[!crps %in% crps_over_sd]
          cuts <- stats::quantile(capridat_dcst[crps_0][capridat_dcst[crps_0] > 0], probs = seq(0, 1, 1/n_cuts), na.rm = T)
          lev_0 <- levels(cut(as.numeric(unlist(capridat_dcst[crps_0])), cuts))
    ...
    #if(page == 1 & !is.null(cuts_const)) cuts_hard <- 1
